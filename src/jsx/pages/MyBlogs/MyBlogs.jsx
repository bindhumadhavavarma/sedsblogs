...
function MyBlogs() {
    ...
    const fetchBlogs = async () => {
        try {
            setIsLoading(true);
            const data = await AxiosPost('fetchMyBlogs.php', { username: localStorage.getItem('username') });
            console.log(data)
            if (data.success) {
                setBlogs(data.blogs)
            }
                                ...
                                <div className="col-12 col-sm-12 col-md-6 col-lg-4 col-xl-4 example-2 card" key={blog.blog_id}>
                                    <div className="wrapper" style={{ "background": " url(https://sedsvitap.in/sedsblogsAPIs/uploads/" + blog.imagelink + ") center/cover no-repeat" }}>
                                        <div className="header">
                                            <div className="date"> <span className="day">day</span> <span className="month">month</span> <span className="year">ear</span>
                                            </div>
                                        </div>
                                        <div className="data">
