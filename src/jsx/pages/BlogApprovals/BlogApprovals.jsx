...
function BlogApprovals() {
    ...
    const fetchBlogs = async () => {
        try {
            setIsLoading(true);
            const data = await AxiosPost('fetchInactiveBlogs.php');
            console.log(data)
            if (data.success) {
                setBlogs(data.blogs)
            }
    ...
    const approveBlog = async (id) => {
        try {
            setIsLoading(true);
            const data = await AxiosPost('approveBlog.php', { blog_id: id });
            console.log(data)
            if (data.success) {
                await fetchBlogs()
    ...
    return (
        <>
            ...
            <section className="Blogs">
                <div className="sectionHeading" style={{ "paddingTop": " 40px", "color": "var(--secondary)" }}>Unapproved Blogs</div>
                <hr />
                                    ...
                                    <div className="wrapper" style={{ "background": " url(https://sedsvitap.in/sedsblogsAPIs/uploads/" + blog.imagelink + ") center/cover no-repeat" }}>
                                        ...
                                        <div className="data">
                                            <div className="content"> <span className="author">{blog.author}</span>
                                                ...
                                                <a href="#" className="button" onClick={() => { approveBlog(blog.blog_id) }}>Approve Blog</a>
